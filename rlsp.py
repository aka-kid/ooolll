...
# 获取ck及视频id
def get_cookie():
    global access_token, user_token, video_id, header, list_id
...
# 分享任务
def share():
    total_url = f'https://ranlv.lvfacn.com/api.php/Rcharts/getRank?access_token={access_token}&basis=0&id={list_id}&list_rows=1&member_id=1408390&page=1&ran=1&user_token={user_token}'
    resp = requests.get(url=total_url, headers=header).json()
    result = str(resp['data']['data'][0]['id'])
    print(result)
    # share_url = f'https://ranlv.lvfacn.com/api.php/Rcharts/shareVideo?access_token={access_token}&member_id=1408390&user_token={user_token}&video_id={votevideo_id}'
    # headers = {
